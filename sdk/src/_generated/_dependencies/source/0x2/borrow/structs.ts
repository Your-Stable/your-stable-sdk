...
export function isReferent(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V34}::borrow::Referent` + '<'); }
...
export function isBorrow(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V34}::borrow::Borrow`; }
