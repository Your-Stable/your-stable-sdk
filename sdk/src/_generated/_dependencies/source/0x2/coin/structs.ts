...
export function isCoin(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V34}::coin::Coin` + '<'); }
...
export function isCoinMetadata(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V34}::coin::CoinMetadata` + '<'); }
...
export class CoinMetadata<T extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V34}::coin::CoinMetadata`; static readonly $numTypeParams = 1; static readonly $isPhantom = [true,] as const;

 readonly $typeName = CoinMetadata.$typeName; readonly $fullTypeName: `${typeof PKG_V34}::coin::CoinMetadata<${PhantomToTypeStr<T>}>`; readonly $typeArgs: [PhantomToTypeStr<T>]; readonly $isPhantom = CoinMetadata.$isPhantom;
 ...
 private constructor(typeArgs: [PhantomToTypeStr<T>], fields: CoinMetadataFields<T>, ) { this.$fullTypeName = composeSuiType( CoinMetadata.$typeName, ...typeArgs ) as `${typeof PKG_V34}::coin::CoinMetadata<${PhantomToTypeStr<T>}>`; this.$typeArgs = typeArgs;
...
export function isRegulatedCoinMetadata(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V34}::coin::RegulatedCoinMetadata` + '<'); }
...
export function isTreasuryCap(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V34}::coin::TreasuryCap` + '<'); }
...
export function isDenyCapV2(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V34}::coin::DenyCapV2` + '<'); }
...
export function isCurrencyCreated(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V34}::coin::CurrencyCreated` + '<'); }
...
export function isDenyCap(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V34}::coin::DenyCap` + '<'); }
