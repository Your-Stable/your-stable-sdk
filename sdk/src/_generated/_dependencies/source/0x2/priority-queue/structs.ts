...
export function isPriorityQueue(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V35}::priority_queue::PriorityQueue` + '<'); }
...
export function isEntry(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V35}::priority_queue::Entry` + '<'); }
