...
export function isToken(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V34}::token::Token` + '<'); }
...
export function isTokenPolicyCap(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V34}::token::TokenPolicyCap` + '<'); }
...
export function isTokenPolicy(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V34}::token::TokenPolicy` + '<'); }
...
export function isActionRequest(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V34}::token::ActionRequest` + '<'); }
...
export class ActionRequest<T extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V34}::token::ActionRequest`; static readonly $numTypeParams = 1; static readonly $isPhantom = [true,] as const;

 readonly $typeName = ActionRequest.$typeName; readonly $fullTypeName: `${typeof PKG_V34}::token::ActionRequest<${PhantomToTypeStr<T>}>`; readonly $typeArgs: [PhantomToTypeStr<T>]; readonly $isPhantom = ActionRequest.$isPhantom;
 ...
 private constructor(typeArgs: [PhantomToTypeStr<T>], fields: ActionRequestFields<T>, ) { this.$fullTypeName = composeSuiType( ActionRequest.$typeName, ...typeArgs ) as `${typeof PKG_V34}::token::ActionRequest<${PhantomToTypeStr<T>}>`; this.$typeArgs = typeArgs;
...
export function isRuleKey(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V34}::token::RuleKey` + '<'); }
...
export function isTokenPolicyCreated(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V34}::token::TokenPolicyCreated` + '<'); }
