...
export function isVecMap(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V35}::vec_map::VecMap` + '<'); }
...
export class VecMap<K extends TypeArgument, V extends TypeArgument> implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V35}::vec_map::VecMap`; static readonly $numTypeParams = 2; static readonly $isPhantom = [false,false,] as const;

 readonly $typeName = VecMap.$typeName; readonly $fullTypeName: `${typeof PKG_V35}::vec_map::VecMap<${ToTypeStr<K>}, ${ToTypeStr<V>}>`; readonly $typeArgs: [ToTypeStr<K>, ToTypeStr<V>]; readonly $isPhantom = VecMap.$isPhantom;

 readonly contents: ToField<Vector<Entry<K, V>>>

 private constructor(typeArgs: [ToTypeStr<K>, ToTypeStr<V>], fields: VecMapFields<K, V>, ) { this.$fullTypeName = composeSuiType( VecMap.$typeName, ...typeArgs ) as `${typeof PKG_V35}::vec_map::VecMap<${ToTypeStr<K>}, ${ToTypeStr<V>}>`; this.$typeArgs = typeArgs;

 this.contents = fields.contents; }

 static reified<K extends Reified<TypeArgument, any>, V extends Reified<TypeArgument, any>>( K: K, V: V ): VecMapReified<ToTypeArgument<K>, ToTypeArgument<V>> { return { typeName: VecMap.$typeName, fullTypeName: composeSuiType( VecMap.$typeName, ...[extractType(K), extractType(V)] ) as `${typeof PKG_V35}::vec_map::VecMap<${ToTypeStr<ToTypeArgument<K>>}, ${ToTypeStr<ToTypeArgument<V>>}>`, typeArgs: [ extractType(K), extractType(V) ] as [ToTypeStr<ToTypeArgument<K>>, ToTypeStr<ToTypeArgument<V>>], isPhantom: VecMap.$isPhantom, reifiedTypeArgs: [K, V], fromFields: (fields: Record<string, any>) => VecMap.fromFields( [K, V], fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => VecMap.fromFieldsWithTypes( [K, V], item, ), fromBcs: (data: Uint8Array) => VecMap.fromBcs( [K, V], data, ), bcs: VecMap.bcs(toBcs(K), toBcs(V)), fromJSONField: (field: any) => VecMap.fromJSONField( [K, V], field, ), fromJSON: (json: Record<string, any>) => VecMap.fromJSON( [K, V], json, ), fromSuiParsedData: (content: SuiParsedData) => VecMap.fromSuiParsedData( [K, V], content, ), fromSuiObjectData: (content: SuiObjectData) => VecMap.fromSuiObjectData( [K, V], content, ), fetch: async (client: SuiClient, id: string) => VecMap.fetch( client, [K, V], id, ), new: ( fields: VecMapFields<ToTypeArgument<K>, ToTypeArgument<V>>, ) => { return new VecMap( [extractType(K), extractType(V)], fields ) }, kind: "StructClassReified", } }
...
export function isEntry(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V35}::vec_map::Entry` + '<'); }
