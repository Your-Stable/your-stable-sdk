...
export function isVersioned(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V34}::versioned::Versioned`; }
...
export class Versioned implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V34}::versioned::Versioned`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = Versioned.$typeName; readonly $fullTypeName: `${typeof PKG_V34}::versioned::Versioned`; readonly $typeArgs: []; readonly $isPhantom = Versioned.$isPhantom;

 readonly id: ToField<UID>; readonly version: ToField<"u64">

 private constructor(typeArgs: [], fields: VersionedFields, ) { this.$fullTypeName = composeSuiType( Versioned.$typeName, ...typeArgs ) as `${typeof PKG_V34}::versioned::Versioned`; this.$typeArgs = typeArgs;

 this.id = fields.id;; this.version = fields.version; }

 static reified( ): VersionedReified { return { typeName: Versioned.$typeName, fullTypeName: composeSuiType( Versioned.$typeName, ...[] ) as `${typeof PKG_V34}::versioned::Versioned`, typeArgs: [ ] as [], isPhantom: Versioned.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Versioned.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Versioned.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => Versioned.fromBcs( data, ), bcs: Versioned.bcs, fromJSONField: (field: any) => Versioned.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => Versioned.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => Versioned.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => Versioned.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => Versioned.fetch( client, id, ), new: ( fields: VersionedFields, ) => { return new Versioned( [], fields ) }, kind: "StructClassReified", } }
...
export function isVersionChangeCap(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V34}::versioned::VersionChangeCap`; }
...
export class VersionChangeCap implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V34}::versioned::VersionChangeCap`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = VersionChangeCap.$typeName; readonly $fullTypeName: `${typeof PKG_V34}::versioned::VersionChangeCap`; readonly $typeArgs: []; readonly $isPhantom = VersionChangeCap.$isPhantom;

 readonly versionedId: ToField<ID>; readonly oldVersion: ToField<"u64">

 private constructor(typeArgs: [], fields: VersionChangeCapFields, ) { this.$fullTypeName = composeSuiType( VersionChangeCap.$typeName, ...typeArgs ) as `${typeof PKG_V34}::versioned::VersionChangeCap`; this.$typeArgs = typeArgs;
