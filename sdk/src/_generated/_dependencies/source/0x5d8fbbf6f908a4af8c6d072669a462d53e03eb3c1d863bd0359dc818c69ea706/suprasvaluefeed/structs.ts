...
export class OracleHolder implements StructClass {
  ...
  private constructor(typeArgs: [], fields: OracleHolderFields) {
    this.$fullTypeName = composeSuiType(
      ...
      ...typeArgs,
    ) as `${typeof PKG_V1}::SupraSValueFeed::OracleHolder`;
  ...
  static reified(): OracleHolderReified {
    return {
      ...
      fromFields: (fields: Record<string, any>) =>
        OracleHolder.fromFields(fields),
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        OracleHolder.fromFieldsWithTypes(item),
      ...
      fromSuiParsedData: (content: SuiParsedData) =>
        OracleHolder.fromSuiParsedData(content),
      fromSuiObjectData: (content: SuiObjectData) =>
        OracleHolder.fromSuiObjectData(content),
      fetch: async (client: SuiClient, id: string) =>
        OracleHolder.fetch(client, id),
  ...
  static fromSuiParsedData(content: SuiParsedData): OracleHolder {
    ...
    if (!isOracleHolder(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a OracleHolder object`,
      );
    }
  ...
  static async fetch(client: SuiClient, id: string): Promise<OracleHolder> {
    ...
    if (res.error) {
      throw new Error(
        `error fetching OracleHolder object at id ${id}: ${res.error.code}`,
      );
    }
    if (
      res.data?.bcs?.dataType !== "moveObject" ||
      !isOracleHolder(res.data.bcs.type)
    ) {
...
export class Price implements StructClass {
  ...
  private constructor(typeArgs: [], fields: PriceFields) {
    this.$fullTypeName = composeSuiType(
      Price.$typeName,
      ...typeArgs,
    ) as `${typeof PKG_V1}::SupraSValueFeed::Price`;
    this.$typeArgs = typeArgs;
