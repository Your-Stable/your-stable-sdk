...
export class Job implements StructClass {
  ...
  static reified(): JobReified {
    return {
      ...
      fromSuiParsedData: (content: SuiParsedData) =>
        Job.fromSuiParsedData(content),
      fromSuiObjectData: (content: SuiObjectData) =>
        Job.fromSuiObjectData(content),
  ...
  static fromSuiParsedData(content: SuiParsedData): Job {
    ...
    if (!isJob(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a Job object`,
      );
    }
  ...
  static async fetch(client: SuiClient, id: string): Promise<Job> {
    ...
    if (res.error) {
      throw new Error(
        `error fetching Job object at id ${id}: ${res.error.code}`,
      );
    }
