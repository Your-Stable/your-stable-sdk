...
export type SwitchboardDecimalReified = Reified<
  SwitchboardDecimal,
...
export class SwitchboardDecimal implements StructClass {
  ...
  private constructor(typeArgs: [], fields: SwitchboardDecimalFields) {
    this.$fullTypeName = composeSuiType(
      ...
      ...typeArgs,
    ) as `${typeof PKG_V1}::math::SwitchboardDecimal`;
  ...
  static reified(): SwitchboardDecimalReified {
    return {
      ...
      fromFields: (fields: Record<string, any>) =>
        SwitchboardDecimal.fromFields(fields),
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        SwitchboardDecimal.fromFieldsWithTypes(item),
      ...
      fromJSON: (json: Record<string, any>) =>
        SwitchboardDecimal.fromJSON(json),
      fromSuiParsedData: (content: SuiParsedData) =>
        SwitchboardDecimal.fromSuiParsedData(content),
      fromSuiObjectData: (content: SuiObjectData) =>
        SwitchboardDecimal.fromSuiObjectData(content),
      fetch: async (client: SuiClient, id: string) =>
        SwitchboardDecimal.fetch(client, id),
  ...
  static fromSuiParsedData(content: SuiParsedData): SwitchboardDecimal {
    ...
    if (!isSwitchboardDecimal(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a SwitchboardDecimal object`,
      );
    }
  ...
  static fromSuiObjectData(data: SuiObjectData): SwitchboardDecimal {
    if (data.bcs) {
      if (
        ...
        !isSwitchboardDecimal(data.bcs.type)
      ) {
  ...
  static async fetch(
    client: SuiClient,
    id: string,
  ): Promise<SwitchboardDecimal> {
    const res = await client.getObject({ id, options: { showBcs: true } });
    if (res.error) {
      throw new Error(
        `error fetching SwitchboardDecimal object at id ${id}: ${res.error.code}`,
      );
    }
    if (
      res.data?.bcs?.dataType !== "moveObject" ||
      !isSwitchboardDecimal(res.data.bcs.type)
    ) {
