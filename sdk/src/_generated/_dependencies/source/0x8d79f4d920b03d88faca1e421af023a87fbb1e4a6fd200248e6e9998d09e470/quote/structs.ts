...
export class Quote implements StructClass {
  ...
  static reified(): QuoteReified {
    return {
      ...
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        Quote.fromFieldsWithTypes(item),
      ...
      fromSuiParsedData: (content: SuiParsedData) =>
        Quote.fromSuiParsedData(content),
      fromSuiObjectData: (content: SuiObjectData) =>
        Quote.fromSuiObjectData(content),
  ...
  static fromFields(fields: Record<string, any>): Quote {
    return Quote.reified().new({
      ...
      verificationTimestamp: decodeFromFields(
        "u64",
  ...
  static fromFieldsWithTypes(item: FieldsWithTypes): Quote {
    ...
    return Quote.reified().new({
      ...
      nodeAuthority: decodeFromFieldsWithTypes(
        "address",
        item.fields.node_authority,
      ),
      queueAddr: decodeFromFieldsWithTypes("address", item.fields.queue_addr),
      quoteBuffer: decodeFromFieldsWithTypes(
        reified.vector("u8"),
        item.fields.quote_buffer,
      ),
      verificationStatus: decodeFromFieldsWithTypes(
        "u8",
        item.fields.verification_status,
      ),
      verificationTimestamp: decodeFromFieldsWithTypes(
        "u64",
        item.fields.verification_timestamp,
      ),
      validUntil: decodeFromFieldsWithTypes("u64", item.fields.valid_until),
      contentHashEnabled: decodeFromFieldsWithTypes(
        "bool",
        item.fields.content_hash_enabled,
      ),
  ...
  static fromJSONField(field: any): Quote {
    return Quote.reified().new({
      ...
      verificationTimestamp: decodeFromJSONField(
        "u64",
  ...
  static fromSuiParsedData(content: SuiParsedData): Quote {
    ...
    if (!isQuote(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a Quote object`,
      );
    }
  ...
  static async fetch(client: SuiClient, id: string): Promise<Quote> {
    ...
    if (res.error) {
      throw new Error(
        `error fetching Quote object at id ${id}: ${res.error.code}`,
      );
    }
    if (
      res.data?.bcs?.dataType !== "moveObject" ||
      !isQuote(res.data.bcs.type)
    ) {
