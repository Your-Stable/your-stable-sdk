...
export interface BottleStrapFields<T extends PhantomTypeArgument> { id: ToField<UID>; feeRate: ToField<Option<"u64">> }

export type BottleStrapReified<T extends PhantomTypeArgument> = Reified< BottleStrap<T>, BottleStrapFields<T> >;

export class BottleStrap<T extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 readonly $typeName = BottleStrap.$typeName; readonly $fullTypeName: `${typeof PKG_V4}::strap::BottleStrap<${PhantomToTypeStr<T>}>`; readonly $typeArgs: [PhantomToTypeStr<T>]; readonly $isPhantom = BottleStrap.$isPhantom;

 readonly id: ToField<UID>; readonly feeRate: ToField<Option<"u64">>

 private constructor(typeArgs: [PhantomToTypeStr<T>], fields: BottleStrapFields<T>, ) { this.$fullTypeName = composeSuiType( BottleStrap.$typeName, ...typeArgs ) as `${typeof PKG_V4}::strap::BottleStrap<${PhantomToTypeStr<T>}>`; this.$typeArgs = typeArgs;

 this.id = fields.id;; this.feeRate = fields.feeRate; }

 static reified<T extends PhantomReified<PhantomTypeArgument>>( T: T ): BottleStrapReified<ToPhantomTypeArgument<T>> { return { typeName: BottleStrap.$typeName, fullTypeName: composeSuiType( BottleStrap.$typeName, ...[extractType(T)] ) as `${typeof PKG_V4}::strap::BottleStrap<${PhantomToTypeStr<ToPhantomTypeArgument<T>>}>`, typeArgs: [ extractType(T) ] as [PhantomToTypeStr<ToPhantomTypeArgument<T>>], isPhantom: BottleStrap.$isPhantom, reifiedTypeArgs: [T], fromFields: (fields: Record<string, any>) => BottleStrap.fromFields( T, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => BottleStrap.fromFieldsWithTypes( T, item, ), fromBcs: (data: Uint8Array) => BottleStrap.fromBcs( T, data, ), bcs: BottleStrap.bcs, fromJSONField: (field: any) => BottleStrap.fromJSONField( T, field, ), fromJSON: (json: Record<string, any>) => BottleStrap.fromJSON( T, json, ), fromSuiParsedData: (content: SuiParsedData) => BottleStrap.fromSuiParsedData( T, content, ), fromSuiObjectData: (content: SuiObjectData) => BottleStrap.fromSuiObjectData( T, content, ), fetch: async (client: SuiClient, id: string) => BottleStrap.fetch( client, T, id, ), new: ( fields: BottleStrapFields<ToPhantomTypeArgument<T>>, ) => { return new BottleStrap( [extractType(T)], fields ) }, kind: "StructClassReified", } }

 static get r() { return BottleStrap.reified }

 static phantom<T extends PhantomReified<PhantomTypeArgument>>( T: T ): PhantomReified<ToTypeStr<BottleStrap<ToPhantomTypeArgument<T>>>> { return phantom(BottleStrap.reified( T )); } static get p() { return BottleStrap.phantom }
 ...
 static fromFields<T extends PhantomReified<PhantomTypeArgument>>( typeArg: T, fields: Record<string, any> ): BottleStrap<ToPhantomTypeArgument<T>> { return BottleStrap.reified( typeArg, ).new( { id: decodeFromFields(UID.reified(), fields.id), feeRate: decodeFromFields(Option.reified("u64"), fields.fee_rate) } ) }

 static fromFieldsWithTypes<T extends PhantomReified<PhantomTypeArgument>>( typeArg: T, item: FieldsWithTypes ): BottleStrap<ToPhantomTypeArgument<T>> { if (!isBottleStrap(item.type)) { throw new Error("not a BottleStrap type");
 ...
 static fromBcs<T extends PhantomReified<PhantomTypeArgument>>( typeArg: T, data: Uint8Array ): BottleStrap<ToPhantomTypeArgument<T>> { return BottleStrap.fromFields( typeArg, BottleStrap.bcs.parse(data) ) }
 ...
 static fromJSONField<T extends PhantomReified<PhantomTypeArgument>>( typeArg: T, field: any ): BottleStrap<ToPhantomTypeArgument<T>> { return BottleStrap.reified( typeArg, ).new( { id: decodeFromJSONField(UID.reified(), field.id), feeRate: decodeFromJSONField(Option.reified("u64"), field.feeRate) } ) }

 static fromJSON<T extends PhantomReified<PhantomTypeArgument>>( typeArg: T, json: Record<string, any> ): BottleStrap<ToPhantomTypeArgument<T>> { if (json.$typeName !== BottleStrap.$typeName) { throw new Error("not a WithTwoGenerics json object") }; assertReifiedTypeArgsMatch( composeSuiType(BottleStrap.$typeName, extractType(typeArg)), json.$typeArgs, [typeArg], )

 return BottleStrap.fromJSONField( typeArg, json, ) }

 static fromSuiParsedData<T extends PhantomReified<PhantomTypeArgument>>( typeArg: T, content: SuiParsedData ): BottleStrap<ToPhantomTypeArgument<T>> { if (content.dataType !== "moveObject") { throw new Error("not an object"); } if (!isBottleStrap(content.type)) { throw new Error(`object at ${(content.fields as any).id} is not a BottleStrap object`); } return BottleStrap.fromFieldsWithTypes( typeArg, content ); }

 static fromSuiObjectData<T extends PhantomReified<PhantomTypeArgument>>( typeArg: T, data: SuiObjectData ): BottleStrap<ToPhantomTypeArgument<T>> { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isBottleStrap(data.bcs.type)) { throw new Error(`object at is not a BottleStrap object`); }
