...
export interface PriceTypeFields<T extends PhantomTypeArgument> { dummyField: ToField<"bool"> }

export type PriceTypeReified<T extends PhantomTypeArgument> = Reified< PriceType<T>, PriceTypeFields<T> >;

export class PriceType<T extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 readonly $typeName = PriceType.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::bucket_oracle::PriceType<${PhantomToTypeStr<T>}>`; readonly $typeArgs: [PhantomToTypeStr<T>]; readonly $isPhantom = PriceType.$isPhantom;

 readonly dummyField: ToField<"bool">

 private constructor(typeArgs: [PhantomToTypeStr<T>], fields: PriceTypeFields<T>, ) { this.$fullTypeName = composeSuiType( PriceType.$typeName, ...typeArgs ) as `${typeof PKG_V1}::bucket_oracle::PriceType<${PhantomToTypeStr<T>}>`; this.$typeArgs = typeArgs;

 this.dummyField = fields.dummyField; }

 static reified<T extends PhantomReified<PhantomTypeArgument>>( T: T ): PriceTypeReified<ToPhantomTypeArgument<T>> { return { typeName: PriceType.$typeName, fullTypeName: composeSuiType( PriceType.$typeName, ...[extractType(T)] ) as `${typeof PKG_V1}::bucket_oracle::PriceType<${PhantomToTypeStr<ToPhantomTypeArgument<T>>}>`, typeArgs: [ extractType(T) ] as [PhantomToTypeStr<ToPhantomTypeArgument<T>>], isPhantom: PriceType.$isPhantom, reifiedTypeArgs: [T], fromFields: (fields: Record<string, any>) => PriceType.fromFields( T, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => PriceType.fromFieldsWithTypes( T, item, ), fromBcs: (data: Uint8Array) => PriceType.fromBcs( T, data, ), bcs: PriceType.bcs, fromJSONField: (field: any) => PriceType.fromJSONField( T, field, ), fromJSON: (json: Record<string, any>) => PriceType.fromJSON( T, json, ), fromSuiParsedData: (content: SuiParsedData) => PriceType.fromSuiParsedData( T, content, ), fromSuiObjectData: (content: SuiObjectData) => PriceType.fromSuiObjectData( T, content, ), fetch: async (client: SuiClient, id: string) => PriceType.fetch( client, T, id, ), new: ( fields: PriceTypeFields<ToPhantomTypeArgument<T>>, ) => { return new PriceType( [extractType(T)], fields ) }, kind: "StructClassReified", } }

 static get r() { return PriceType.reified }

 static phantom<T extends PhantomReified<PhantomTypeArgument>>( T: T ): PhantomReified<ToTypeStr<PriceType<ToPhantomTypeArgument<T>>>> { return phantom(PriceType.reified( T )); } static get p() { return PriceType.phantom }
 ...
 static fromFields<T extends PhantomReified<PhantomTypeArgument>>( typeArg: T, fields: Record<string, any> ): PriceType<ToPhantomTypeArgument<T>> { return PriceType.reified( typeArg, ).new( { dummyField: decodeFromFields("bool", fields.dummy_field) } ) }

 static fromFieldsWithTypes<T extends PhantomReified<PhantomTypeArgument>>( typeArg: T, item: FieldsWithTypes ): PriceType<ToPhantomTypeArgument<T>> { if (!isPriceType(item.type)) { throw new Error("not a PriceType type");

 } assertFieldsWithTypesArgsMatch(item, [typeArg]);

 return PriceType.reified( typeArg, ).new( { dummyField: decodeFromFieldsWithTypes("bool", item.fields.dummy_field) } ) }

 static fromBcs<T extends PhantomReified<PhantomTypeArgument>>( typeArg: T, data: Uint8Array ): PriceType<ToPhantomTypeArgument<T>> { return PriceType.fromFields( typeArg, PriceType.bcs.parse(data) ) }
 ...
 static fromJSONField<T extends PhantomReified<PhantomTypeArgument>>( typeArg: T, field: any ): PriceType<ToPhantomTypeArgument<T>> { return PriceType.reified( typeArg, ).new( { dummyField: decodeFromJSONField("bool", field.dummyField) } ) }

 static fromJSON<T extends PhantomReified<PhantomTypeArgument>>( typeArg: T, json: Record<string, any> ): PriceType<ToPhantomTypeArgument<T>> { if (json.$typeName !== PriceType.$typeName) { throw new Error("not a WithTwoGenerics json object") }; assertReifiedTypeArgsMatch( composeSuiType(PriceType.$typeName, extractType(typeArg)), json.$typeArgs, [typeArg], )

 return PriceType.fromJSONField( typeArg, json, ) }

 static fromSuiParsedData<T extends PhantomReified<PhantomTypeArgument>>( typeArg: T, content: SuiParsedData ): PriceType<ToPhantomTypeArgument<T>> { if (content.dataType !== "moveObject") { throw new Error("not an object"); } if (!isPriceType(content.type)) { throw new Error(`object at ${(content.fields as any).id} is not a PriceType object`); } return PriceType.fromFieldsWithTypes( typeArg, content ); }

 static fromSuiObjectData<T extends PhantomReified<PhantomTypeArgument>>( typeArg: T, data: SuiObjectData ): PriceType<ToPhantomTypeArgument<T>> { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isPriceType(data.bcs.type)) { throw new Error(`object at is not a PriceType object`); }
