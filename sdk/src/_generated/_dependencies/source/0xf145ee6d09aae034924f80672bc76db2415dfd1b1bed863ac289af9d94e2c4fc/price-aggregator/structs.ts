...
export function isPriceInfo(type: string): boolean {
  ...
  return type === `${PKG_V1}::price_aggregator::PriceInfo`;
}
...
export class PriceInfo implements StructClass {
  ...
  static readonly $typeName = `${PKG_V1}::price_aggregator::PriceInfo`;
  static readonly $numTypeParams = 0;
  static readonly $isPhantom = [] as const;

  readonly $typeName = PriceInfo.$typeName;
  readonly $fullTypeName: `${typeof PKG_V1}::price_aggregator::PriceInfo`;
  readonly $typeArgs: [];
  ...
  private constructor(typeArgs: [], fields: PriceInfoFields) {
    this.$fullTypeName = composeSuiType(
      PriceInfo.$typeName,
      ...typeArgs,
    ) as `${typeof PKG_V1}::price_aggregator::PriceInfo`;
...
export function isPriceVector(type: string): boolean {
  type = compressSuiType(type);
  return type === `${PKG_V1}::price_aggregator::PriceVector`;
}

export interface PriceVectorFields {
  vec: ToField<Vector<"u64">>;
}

export type PriceVectorReified = Reified<PriceVector, PriceVectorFields>;

export class PriceVector implements StructClass {
  __StructClass = true as const;

  static readonly $typeName = `${PKG_V1}::price_aggregator::PriceVector`;
  static readonly $numTypeParams = 0;
  static readonly $isPhantom = [] as const;

  readonly $typeName = PriceVector.$typeName;
  readonly $fullTypeName: `${typeof PKG_V1}::price_aggregator::PriceVector`;
  readonly $typeArgs: [];
  readonly $isPhantom = PriceVector.$isPhantom;

  readonly vec: ToField<Vector<"u64">>;

  private constructor(typeArgs: [], fields: PriceVectorFields) {
    this.$fullTypeName = composeSuiType(
      PriceVector.$typeName,
      ...typeArgs,
    ) as `${typeof PKG_V1}::price_aggregator::PriceVector`;
    this.$typeArgs = typeArgs;

    this.vec = fields.vec;
  }

  static reified(): PriceVectorReified {
    return {
      typeName: PriceVector.$typeName,
      fullTypeName: composeSuiType(
        PriceVector.$typeName,
        ...[],
      ) as `${typeof PKG_V1}::price_aggregator::PriceVector`,
      typeArgs: [] as [],
      isPhantom: PriceVector.$isPhantom,
      reifiedTypeArgs: [],
      fromFields: (fields: Record<string, any>) =>
        PriceVector.fromFields(fields),
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        PriceVector.fromFieldsWithTypes(item),
      fromBcs: (data: Uint8Array) => PriceVector.fromBcs(data),
      bcs: PriceVector.bcs,
      fromJSONField: (field: any) => PriceVector.fromJSONField(field),
      fromJSON: (json: Record<string, any>) => PriceVector.fromJSON(json),
      fromSuiParsedData: (content: SuiParsedData) =>
        PriceVector.fromSuiParsedData(content),
      fromSuiObjectData: (content: SuiObjectData) =>
        PriceVector.fromSuiObjectData(content),
      fetch: async (client: SuiClient, id: string) =>
        PriceVector.fetch(client, id),
      new: (fields: PriceVectorFields) => {
        return new PriceVector([], fields);
      },
      kind: "StructClassReified",
    };
  }
  ...
  static get r() {
    return PriceVector.reified();
  }

  static phantom(): PhantomReified<ToTypeStr<PriceVector>> {
    return phantom(PriceVector.reified());
  }
  static get p() {
    return PriceVector.phantom();
  }

  static get bcs() {
    return bcs.struct("PriceVector", {
      vec: bcs.vector(bcs.u64()),
    });
  }

  static fromFields(fields: Record<string, any>): PriceVector {
    return PriceVector.reified().new({
      vec: decodeFromFields(reified.vector("u64"), fields.vec),
    });
  }

  static fromFieldsWithTypes(item: FieldsWithTypes): PriceVector {
    if (!isPriceVector(item.type)) {
      throw new Error("not a PriceVector type");
    }

    return PriceVector.reified().new({
      vec: decodeFromFieldsWithTypes(reified.vector("u64"), item.fields.vec),
    });
  }

  static fromBcs(data: Uint8Array): PriceVector {
    return PriceVector.fromFields(PriceVector.bcs.parse(data));
  }
  ...
  static fromJSONField(field: any): PriceVector {
    return PriceVector.reified().new({
      vec: decodeFromJSONField(reified.vector("u64"), field.vec),
    });
  }

  static fromJSON(json: Record<string, any>): PriceVector {
    if (json.$typeName !== PriceVector.$typeName) {
      throw new Error("not a WithTwoGenerics json object");
    }
  ...
  static fromSuiParsedData(content: SuiParsedData): PriceVector {
    if (content.dataType !== "moveObject") {
      throw new Error("not an object");
    }
    if (!isPriceVector(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a PriceVector object`,
      );
    }
    return PriceVector.fromFieldsWithTypes(content);
  }

  static fromSuiObjectData(data: SuiObjectData): PriceVector {
    if (data.bcs) {
      if (data.bcs.dataType !== "moveObject" || !isPriceVector(data.bcs.type)) {
        throw new Error(`object at is not a PriceVector object`);
      }

      return PriceVector.fromBcs(fromB64(data.bcs.bcsBytes));
    }
    if (data.content) {
      return PriceVector.fromSuiParsedData(data.content);
    }
    throw new Error(
      "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.",
    );
  ...
  static async fetch(client: SuiClient, id: string): Promise<PriceVector> {
    const res = await client.getObject({ id, options: { showBcs: true } });
    if (res.error) {
      throw new Error(
        `error fetching PriceVector object at id ${id}: ${res.error.code}`,
      );
    }
    if (
      res.data?.bcs?.dataType !== "moveObject" ||
      !isPriceVector(res.data.bcs.type)
    ) {
      throw new Error(`object at id ${id} is not a PriceVector object`);
    }
