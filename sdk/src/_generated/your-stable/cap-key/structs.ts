...
export function isCapKey(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::cap_key::CapKey`; }
...
export class CapKey implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V1}::cap_key::CapKey`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = CapKey.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::cap_key::CapKey`; readonly $typeArgs: []; readonly $isPhantom = CapKey.$isPhantom;

 readonly dummyField: ToField<"bool">

 private constructor(typeArgs: [], fields: CapKeyFields, ) { this.$fullTypeName = composeSuiType( CapKey.$typeName, ...typeArgs ) as `${typeof PKG_V1}::cap_key::CapKey`; this.$typeArgs = typeArgs;

 this.dummyField = fields.dummyField; }

 static reified( ): CapKeyReified { return { typeName: CapKey.$typeName, fullTypeName: composeSuiType( CapKey.$typeName, ...[] ) as `${typeof PKG_V1}::cap_key::CapKey`, typeArgs: [ ] as [], isPhantom: CapKey.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => CapKey.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => CapKey.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => CapKey.fromBcs( data, ), bcs: CapKey.bcs, fromJSONField: (field: any) => CapKey.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => CapKey.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => CapKey.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => CapKey.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => CapKey.fetch( client, id, ), new: ( fields: CapKeyFields, ) => { return new CapKey( [], fields ) }, kind: "StructClassReified", } }
